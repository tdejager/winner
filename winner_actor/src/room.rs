...
pub struct Room {
    ...
    winners: HashMap<Winner, Recipient<crate::messages::server::RoomStateChanged>>,
}
...
mod test {
    ...
    use crate::room::{Room, RoomState, RoomVotingState};
    use actix::{
        Actor, ActorContext, ActorFutureExt, Addr, AsyncContext, Context, Handler, Message,
        MessageResult, ResponseActFuture, Running, WrapFuture,
    };
    ...
    async fn vote_on_story() {
        ...
        let state = client.send(GetStateMessage).await.unwrap().unwrap();
        assert!(matches!(state.voting_state, RoomVotingState::Idle));
    }

    #[actix::test]
    async fn vote_on_story_with_fight() {
        let (room, client, ..) = test_setup().await;
        ...
        let state: RoomState = client.send(GetStateMessage).await.unwrap().unwrap();
        matches!(state.voting_state, RoomVotingState::Voting(_));

        let votes = [StoryPoints::ONE, StoryPoints::EIGHT];
        // Vote for both winners
        for entry in state.winners.iter().zip(votes.iter()) {
            let (winner, points) = entry;
            room.send(crate::messages::client::Vote {
                winner: winner.clone(),
                story_id: story.id.clone(),
                points: points.clone(),
        ...
        let state = client.send(GetStateMessage).await.unwrap().unwrap();
        assert!(matches!(state.voting_state, RoomVotingState::Fighting(_)));
        room.send(crate::messages::client::RestartVote {})
            .await
            .unwrap()
            .unwrap();

        // Should be voting again
        let state = client.send(GetStateMessage).await.unwrap().unwrap();
        matches!(state.voting_state, RoomVotingState::Voting(_));
    }
}
